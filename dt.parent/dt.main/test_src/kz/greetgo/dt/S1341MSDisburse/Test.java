package kz.greetgo.dt.S1341MSDisburse;
...
public class Test {
  ...
  public void startBatch() throws IOException, URISyntaxException {
    ...
    scope.put("client.account.0.sumOverPaid", new Num(BigDecimal.valueOf(100)));
    scope.put("client.account.0.returnOverpayment", new Bool(true));


    String code = resource("S1341MSDisburse.txt");
    m.registerTree("main", code);
    m.registerUserProcedure("checkValidity", resource("checkValidity.txt"));
    m.registerUserProcedure("overpaymentReturnSuccessful", resource("overpaymentReturnSuccessful.txt"));
